...
int main() {
    ...
    int imageWidth = 100;
    int imageHeight = 100;
    const unsigned short palette[2] = {0X000,0XFFF};
    ...
    // Load the palette
    LoadRGB4(&screen->ViewPort, (UWORD *)palette, 2);

    // Clear the screen
    SetRast(&screenContext, 0);

    // open file
    fileHandle = fopen("2col.planes", "rb");
    if (fileHandle == NULL) {
    ...
    // to keep it simple we only use 1 plane, so the plane size is the same as the file size
    planeSize = fileSize;
    ...
    InitRastPort(&ctx);
    ctx.BitMap = &bitmap;
    InitBitMap(&bitmap, 1, imageWidth, imageHeight);
    for (i = 0; i < 1; i++) {
    ...
    // one would assume this would work, but it doesn't
    // bitmap.Planes[i] contains a pointer to chip memory, right ?
    CopyMem(fastMemory, bitmap.Planes[i], planeSize);
    FreeMem(fastMemory, fileSize);
